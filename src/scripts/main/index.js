(function () {
  ...
  function onAppReady() {
    ...
    Settings.loadSettings(getConfigPath())
  ...
  function importStyle() {
    dialog.showOpenDialog(
      window,
      getOpenDialogOptions("Import Style", app.getPath("desktop"), ["css"]),
      function (fileNames) {
        if (fileNames instanceof Array) {
          for (var fileName of fileNames) {
            Settings.addStyle(fileName, path.join(getStylePath())).catch(
              onError,
            );
  ...
  function onWindowLoaded(evt) {
    for (var style of Settings.getStyles())
      window.webContents.send("load-style", style);
  }
